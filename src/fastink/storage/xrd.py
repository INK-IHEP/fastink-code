...
async def list_path(
    ...
    try:
        dname = unquote_expand_user(dname = dname, username = username, url = False)
        if dname[0:4] == "/afs":
            logger.debug(f"Target is afs. executing aklog.")
            ret = subprocess.check_output(f"aklog", env=env, shell=True, timeout=2, encoding="UTF-8")
            logger.debug(f"aklog log: {ret}")
            ret = subprocess.check_output(f"klist", env=env, shell=True, timeout=2, encoding="UTF-8")
            logger.debug(f"klist log:\n{ret}")
        cmd.append(f'''{dname}''') if '"' in dname else cmd.append(f"""{dname}""")
...
async def delete_path(
    ...
    if path_type == PathType.DIR:
        raw_files = await list_path(dname = name, username = username, long=True, recursive=True, showhidden = True, mgm = mgm)
        for f in raw_files:
            if f["type"] == "directory":
                dirs.append(f["path"])
            else:
