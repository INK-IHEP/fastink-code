...
def resolve_tgt(ccache_file: str):
    ...
    expired_at_int = int(
        datetime.strptime(
            f"{expired_at[0]} {expired_at[1]}", "%m/%d/%y %H:%M:%S"
        ).timestamp()
    )
    renew_until_int = int(
        datetime.strptime(
            f"{renew_until[0]} {renew_until[1]}", "%m/%d/%y %H:%M:%S"
        ).timestamp()
    )
    return {
        "username": username,
...
def get_krb5(
    ...
    """Get a kerberos token by username or uid. If the token is expiring, it will be renewed.
    ...
    try:
        ...
        raise ValueError(f"User not found (username={username!r}, uid={uid!r})")
    user_id = user_item["id"]
