...
def get_user_jobs_dir(username: str, uid: Optional[int] = None) -> str:
    """Return the configured ``.ink/Jobs`` directory for a user."""
    return f"{get_user_ink_dir(username, uid)}/.ink/Jobs"
...
async def init_job_dir(username: str, job_type: str):
    ...
    job_dir = f"{get_user_jobs_dir(username, uid)}/{job_type}-{time_stamp}"
...
async def init_sync_job_dir(username: str, job_type: str, job_dir: Optional[str] = None, script_path: Optional[str] = None) -> str:
    ...
    def build_default_job_dir() -> str:
        time_stamp = datetime.now().strftime("%Y%m%d-%H%M%S")
        return f"{get_user_jobs_dir(username, uid)}/{job_type}-{time_stamp}"
