...
from fastink.computing.tools.common.slurm_states import SLURM_TERMINAL_JOB_STATUSES
...
def find_active_jobs(uid, jobtype, clusterid=None, *, session:Session):
    ...
    stmt = stmt.where(models.JobInfo.job_status.notin_(SLURM_TERMINAL_JOB_STATUSES | {"CANCELED"}))
...
def get_active_cluster_jobs(
    ...
    *,
    ...
    stmt = (
        ...
        .where(
            or_(
                models.JobInfo.job_status.notin_(SLURM_TERMINAL_JOB_STATUSES | {"CANCELED"}),
                models.JobInfo.job_end_time.is_(None),
