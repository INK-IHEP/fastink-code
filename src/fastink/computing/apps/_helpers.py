...
async def read_latest_job_login_info(
    *,
    ...
    When ``cluster_id`` is given, primarily searches the cluster-scoped
    subdirectory ``<ink_dir>/.ink/Jobs/<cluster_id>``; we also fall back to
    the legacy flat ``<ink_dir>/.ink/Jobs`` (for pre-migration job dirs
    created before the cluster split landed) and pick the newest across
    both locations so long-running old sessions remain discoverable.
    """
    xrootd_path = get_config("storage", "xrd_host")

    async def _list(scope_dir: str) -> list[str]:
        # Explicit exists-check keeps a missing directory (expected on
        # first-time use of a fresh cluster subdir) as an empty listing,
        # without swallowing real auth / network / xrootd errors that
        # would come out of list_path.
        exists, _ = await _storage_common.path_exist(
            name=scope_dir, username=username, mgm=xrootd_path,
        )
        if not exists:
            return []
        entries = await _storage_common.list_path(
            dname=scope_dir, username=username, mgm=xrootd_path,
        )
    ...
    candidates: list[str] = await _list(get_user_jobs_dir(username, uid, cluster_id))
    if cluster_id:
        # Also look in the legacy flat parent so pre-migration jobs that
        # this user might still hold open are visible to connect fallbacks.
        candidates.extend(await _list(get_user_jobs_dir(username, uid, None)))

    if not candidates:
        return None

    # Sort by the timestamp suffix (basename), not full path, so a fresh
    # entry under .ink/Jobs/htcondor/ correctly beats an old one under
    # the flat .ink/Jobs/ even though their prefixes differ.
