...
INK_SLURM_API_JOB_TYPES = {"common"}


def _get_returnable_slurm_job_types() -> set[str]:
    return set(get_config("jobtype").keys()) | INK_SLURM_API_JOB_TYPES


def _should_track_slurm_job_type(
    job_type: object,
    ...
    if returnable_job_types is None:
        returnable_job_types = _get_returnable_slurm_job_types()

    return job_type in returnable_job_types
...
async def _process_single_job(r, cluster: str, raw_job: str, job_number: int):
    ...
    try:
        ...
        try:
            ...
            logger.debug(f"[{cluster}] Pop async job #{job_number}: {job}")
            ...
            if isinstance(job, dict):
                ...
                if retry_count <= max_retries:
                    ...
                    # Leave a short-lived per-user FAILED record so the job
                    # doesn't just vanish from the user's list (the global
                    # failed_jobs:{cluster} dead-letter above is ops-only;
                    ...
                    try:
                        await record_failed_job(
                            ...
                            reason=str(e),
                            req_cpu=str(job.get("cpu", "")),
                            req_mem=str(job.get("mem", "")),
                            req_os=str(job.get("os", "")),
                        )
                    except Exception as rec_err:
                        logger.error(f"[{cluster}] Failed to record failed job: {rec_err}")
                    logger.error(
...
async def slurm_update_job_state(cluster: str):
    ...
    sacct_cmd = (
        "sacct -S now-1day "
        "--format=JobIDRaw,User,Partition,State,Elapsed,"
    ...
    for line in lines:
        ...
        fields = line.split("|")
        if len(fields) < 14:
            logger.debug("slurm_update_job_state: skip malformed sacct line: %s", line)
            continue

        job_id = fields[0].strip()
        job_type = fields[7].strip()

        if not _should_track_slurm_job_type(job_type, returnable_job_types):
            continue
    ...
    for job_id, info in slurm_jobs.items():
        ...
        if job_id not in db_job_map:
            ...
            # outpath/errpath are NOT NULL columns; sbatch without --output leaves them None
            out_path, err_path = parse_sbatch_out_err(
                info["submit_line"],
                job_id,
            )
            out_path = out_path or f"slurm-{job_id}.out"
            err_path = err_path or f"slurm-{job_id}.err"
...
async def slurm_update_job_time(cluster: str):
    ...
    sacct_cmd = (
        "sacct -S now-1day "
        "--format=JobIDRaw,User,Start,End "
        "-P -X -n"
    )
    ...
    for line in lines:
        ...
        fields = line.split("|")
        if len(fields) < 4:
            logger.debug("slurm_update_job_time: skip malformed sacct line: %s", line)
            continue

        job_id = fields[0].strip()
        username = fields[1].strip()
        start_time = normalize_slurm_text_field(fields[2])
        end_time = normalize_slurm_text_field(fields[3])

        uid = change_username_to_uid(username)
        if not uid:
            continue

        if not job_exists(job_id, cluster):
            continue
