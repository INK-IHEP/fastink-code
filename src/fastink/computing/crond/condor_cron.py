...
from fastink.computing.tools.common.failed_jobs import record_failed_job
...
# In-flight compensating condor_rm tasks, keyed by "{owner}:{cluster_id}".
# Serves two purposes: (a) holds a strong reference so the fire-and-forget
# task is not garbage-collected mid-run, and (b) deduplicates -- the 5s
# refresh loop must not stack a new condor_rm for the same job while the
# previous attempt (30s budget) is still running.
_compensating_rms: dict = {}


def _spawn_compensating_condor_rm(job_owner: str, job_clusterid: str):
    key = f"{job_owner}:{job_clusterid}"
    if key in _compensating_rms:
        return  # previous attempt still in flight

    async def _run():
        try:
            logger.info(
                f"HTC-CROND-QUEUE-LOG: tombstoned job {job_clusterid} "
                f"(owner={job_owner}) still in condor_q; retrying condor_rm"
            )
            SCHEDD_HOST = get_config("computing", "schedd_host")
            CM_HOST = get_config("computing", "cm_host")
            rm_cmd = (
                f"sudo -u {job_owner} condor_rm "
                f"-name {quote(SCHEDD_HOST)} -pool {quote(CM_HOST)} {job_clusterid}"
            )
            await sub_command(rm_cmd, 30, "compensating condor_rm failed", "compensating condor_rm timeout")
            logger.info(f"HTC-CROND-QUEUE-LOG: compensating condor_rm {job_clusterid} succeeded")
        except Exception as e:
            logger.warning(
                f"HTC-CROND-QUEUE-LOG: compensating condor_rm {job_clusterid} "
                f"failed ({e}); will retry next sweep while tombstone holds"
            )
        finally:
            _compensating_rms.pop(key, None)

    _compensating_rms[key] = asyncio.ensure_future(_run())
...
async def refresh_redis_job_status():
    try:
        ...
        for job in condor_q_jobs:
            ...
            if tomb:
                # User deleted this job but it is still visible in condor_q:
                # the original condor_rm may have timed out against a slow
                # schedd.  Compensate: retry the removal in the background
                # (fire-and-forget, one attempt per sweep) so a slow schedd
                # or multiple tombstoned jobs cannot block this 5s-interval
                # refresh loop for up to 30s each.
                _spawn_compensating_condor_rm(job_owner, job_clusterid)
                continue
...
async def submit_job_from_redis():
    try:   
        ...
        while True:
            raw_job = None
            # Defensive defaults: if json.loads or an early step throws,
            # the except block below still has bound names to work with.
            job = None
            job_owner = ""
            job_type = ""
            cluster_id = ""
            try:
                ...
                job_dir = await init_job_dir(job_owner, job_type, cluster_id=cluster_id or "htcondor")
                ...
                stdout = await sub_command(submit_command, 20, "submit job failed.", "submit job timeout.")
                ...
                if raw_job and job_owner:
                    await r.lrem(f"{job_owner}_submitting_jobs", 1, raw_job)
                    # Leave a short-lived FAILED record so the user can see
                    # what happened from the job list / details API instead
                    # of the job silently vanishing (visible via query_job
                    # with jobStatus=FAILED; full reason via get_joboutput).
                    try:
                        await record_failed_job(
                            r,
                            username=job_owner,
                            cluster_id=cluster_id or "htcondor",
                            job_type=job_type or "",
                            reason=str(e),
                            req_cpu=job.get("jobReqCPU", "") if isinstance(job, dict) else "",
                            req_mem=job.get("jobReqMEM", "") if isinstance(job, dict) else "",
                            req_os=job.get("jobReqOS", "") if isinstance(job, dict) else "",
                        )
                    except Exception as rec_err:
                        logger.error(f"HTC-ASYNC-LOG: Failed to record failed job: {rec_err}")
