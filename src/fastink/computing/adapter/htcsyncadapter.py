...
class HTC_SYNC_Scheduler(SchedulerBase):
    ...
    async def submit_job(self, htc_job_params: HTC_JOB) -> str:
        try:
            # cluster_id is hard-coded to "htcondor" (not self.CLUSTER_TYPE)
            # so the job dir lands under .ink/Jobs/htcondor/ alongside the
            # main async-submitted htcondor jobs.  self.CLUSTER_TYPE is
            # "htcondor-sync", an internal namespacing label used for DB /
            # Redis keys, not the user-facing cluster identity.
            job_dir = await init_job_dir(self.USERNAME, htc_job_params.job_type, cluster_id="htcondor")
            ...
            for line in lines:
                ...
                if job_param_list[5] == '1':    
                    ...
                    if connect_sign == "False":
                        ...
                        job_start_keywords = computing_registry.start_keywords_for(HepJob_JobType)
