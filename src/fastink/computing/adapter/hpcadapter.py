...
class HPC_Scheduler(SchedulerBase):
    ...
    def _seconds_to_slurm_time(self, total_seconds: int) -> str:
        return seconds_to_slurm_time(total_seconds)

    def _normalize_slurm_time_limit(self, raw_value: object, fallback: str = "24:00:00") -> str:
        return normalize_slurm_time_limit(raw_value, fallback)
    ...
    def _get_returnable_job_types(self) -> set[str]:
        return set(self._get_interactive_job_types()) | {"common"}

    def _requested_job_types(
        self,
        req_job_type: Optional[str],
    ) -> Optional[set[str]]:
        if not req_job_type:
            return None
        ...
        requested = {
            job_type.strip()
            for job_type in req_job_type.split(",")
            if job_type.strip()
        }
    ...
    def _should_return_slurm_job_type(
        self,
        job_type: Optional[str],
        requested_job_types: Optional[set[str]],
        returnable_job_types: set[str],
    ) -> bool:
        job_type = (job_type or "").strip()
        if not job_type or job_type not in returnable_job_types:
            return False
        return requested_job_types is None or job_type in requested_job_types
    ...
    async def _gen_slurm_submit_cmd(
        ...
        has_explicit_time = time is not None and bool(str(time).strip())
        interactive_job_types = self._get_interactive_job_types()
        if jobtype in interactive_job_types:
            if has_explicit_time:
                interactive_job_time_limit = time
            else:
            ...
            # Shared shell.sh at <cluster_scripts>/apps/shell.sh (framework
            # asset colocated with the JobApp abstraction it drives).
            # Per-app run.sh comes from the app registry so plugin-provided
            # apps work too.
            ...
            _app = _computing_registry.try_get(jobtype)
            job_script = str(_app.run_script_path()) if _app else f"{executable_dir}/apps/{jobtype}/run.sh"
            ...
            if has_explicit_time:
                args.append(f"--time={self._normalize_slurm_time_limit(time)}")
            if job_content:
    ...
    async def submit_job_sync(self, hpc_job_params: SLURM_JOB) -> dict:
        try:
            job_dir = await init_job_dir(self.USERNAME, hpc_job_params.job_type, cluster_id=self.CLUSTER_TYPE)
    ...
    async def submit_job_from_queue(self, job: dict) -> dict:
        ...
        try:
            ...
            # --------------------------------------------------
            job_dir = await init_job_dir(username, job_type, cluster_id=self.CLUSTER_TYPE)
    ...
    async def query_job(self, req_job_type: Optional[str] = None):
        ...
        job_list = []
        iptables_jobtype = computing_registry.iptables_jobtypes()
        returnable_job_types = self._get_returnable_job_types()
        requested_job_types = self._requested_job_types(req_job_type)
        ...
        for line in lines:
            ...
            fields = line.split("|")
            if len(fields) < 13:
                logger.debug("query_job: skip malformed sacct line: %s", line)
                continue

            job_id = fields[0].strip()
            partition = fields[1]
            slurm_state = fields[2]
            job_status = normalize_slurm_state(slurm_state)
            node_list = fields[5]
            job_type = fields[6].strip()

            if not self._should_return_slurm_job_type(
                job_type,
                requested_job_types,
                returnable_job_types,
            ):
                continue
            submit_time = normalize_slurm_text_field(fields[7])
            start_time = normalize_slurm_text_field(fields[8])
            end_time = normalize_slurm_text_field(fields[9])
            submit_time = submit_time.replace("T", " ") if submit_time else ""
            start_time = start_time.replace("T", " ") if start_time else ""
            end_time = end_time.replace("T", " ") if end_time else ""
            workdir = fields[10]
            ...
            if not job_status:
                logger.debug(
                    "query_job: skip unrecognized slurm state job_id=%s raw_state=%s",
                    job_id,
                    slurm_state,
                )
                continue
            ...
            if job_status == "RUNNING":
                if connect_sign == "False":
                    ...
                    if any(kw in output_content for kw in computing_registry.start_keywords_for(job_type)):
                        connect_sign = "True"
            ...
            elif job_status in SLURM_TERMINAL_JOB_STATUSES:
                if end_time and not get_endtime_info(self.UID, job_id, self.CLUSTER_TYPE):
                    update_end_time(
        ...
        # ======================================================
        # 2b. Append recently FAILED submissions (self-expiring 24h
        #     records written by the slurm cron worker after the retry
        #     budget is exhausted).  hold_reason carries a truncated
        #     failure reason; full text via get_joboutput with failId.
        # ======================================================
        for record in await list_failed_jobs(r, self.USERNAME):
            ...
            if not self._should_return_slurm_job_type(
                ...
                continue
            job_list.append(failed_job_query_entry(self.CLUSTER_TYPE, record))
    ...
    async def cancel_job(self, job_id: Optional[int] = None, submit_uuid: Optional[str] = None):
        ...
        # above any real Slurm job id).  Deleting one just dismisses the
        # record -- there is no cluster job to scancel.
        if job_id and await delete_failed_job(r, self.USERNAME, job_id):
            logger.debug(f"SLURM-LOG: dismissed failed-submission record {job_id}")
            return {"status": "DELETED", "job_id": job_id}
        # Idempotent dismiss: ids in the pseudo range whose record already
        # expired (24h TTL) are a successful no-op, not a scancel attempt.
        try:
            if job_id is not None and int(job_id) > 10**12:
                logger.debug(f"SLURM-LOG: failed-record {job_id} already expired; no-op")
                return {"status": "DELETED", "job_id": job_id}
    ...
    async def cancel_job(
        ...
        *,
        ...
        if job_id:
            deleted_failed_record = await delete_failed_job(r, username, job_id)
            if deleted_failed_record:
                logger.debug(
                    "cancel_job: dismissed failed-submission record %s",
                    job_id,
                )
                return {
                    "cluster": cluster,
