...
from fastink.computing.hpc.v2.hpc_job_types import (
    ...
    requested_hpc_job_types,
)
from fastink.computing.tools.common.utils import change_uid_to_username, create_iptables, delete_iptables, sub_command
from fastink.computing.tools.db.db_tools import get_job_info, insert_job_info, update_job_status, update_connect_status, get_job_connect_info, get_job_iptables_status
...
async def get_user_jobs(uid, job_type, cluster_id):
    try:
        ...
        command = (f"sacct -u {username} --format=JobID,Partition,JobName,User,State,Elapsed,NNodes,NodeList,WCKey,AdminComment,Start,Submit,WorkDir -P")
        ...
        requested_job_types = requested_hpc_job_types(job_type)
        ...
        for line in lines[1:]:
            ...
            if not ink_job_type:
                continue
            if requested_job_types is not None and ink_job_type not in requested_job_types:
                continue
            ...
            if job_data["NodeList"] == "None assigned":
                ...
                # Optional site-wide DNS suffix appended to bare node names
                node_suffix = get_config("computing", "node_domain_suffix", fallback="")
                if node_suffix:
                    job_data["NodeList"] = f"{job_data['NodeList']}{node_suffix}"
