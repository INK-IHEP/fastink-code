...
async def get_total_resources():
    ...
    # Optional partition filter; empty list means count all partitions
    counted_partitions = get_config("computing", "system_info_partitions", fallback=[]) or []
    for node in nodes:
        partition_match = re.search(r'Partitions=(\S+)', node)
        if counted_partitions and partition_match and partition_match.group(1) not in counted_partitions:
            continue
        if 'CPUTot=' in node:
