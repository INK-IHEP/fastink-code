...
from fastink.computing.hpc.v2.hpc_job_types import get_sacct_field, normalize_hpc_job_type
from fastink.computing.tools.common.utils import change_uid_to_username, create_iptables, delete_iptables, sub_command
from fastink.computing.tools.db.db_tools import get_job_info, insert_job_info, update_job_status, get_job_connect_info, update_connect_status, get_job_iptables_status
...
async def get_user_jobs(uid, jobId, cluster_id):
    try:
        ...
        command = f"su {user_name} -c 'sacct -j {jobId} --format=JobID,Partition,JobName,User,State,Elapsed,NNodes,NodeList,WCKey,AdminComment,Start,Submit,WorkDir -P'"
        ...
        return job_data.get("Start"), job_data.get("State"), job_data.get("NodeList"), job_data.get("Submit"), normalize_hpc_job_type(
            get_sacct_field(job_data, "WCKey", "WCkey", "WCKEY"),
            get_sacct_field(job_data, "AdminComment"),
        ), connect_sign
