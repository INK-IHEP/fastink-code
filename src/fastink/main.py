...
from fastink.auth.oidc import should_mount_oidc
...
from fastink.routers.v2 import admin_manager as admin_manager_v2
...
from fastink.routers.v3 import auth as auth_v3
from fastink.routers.v3 import credentials as credentials_v3
from fastink.routers.v3 import files as files_v3
from fastink.routers.v3 import jobs as jobs_v3
from fastink.routers.v3 import status as status_v3
from fastink.routers.v3.middleware import BearerAuthMiddleware

app.add_middleware(
    BearerAuthMiddleware,
    public_paths=["/api/v3/status/health"],
)
app.include_router(auth_v3.router, prefix="/api/v3")
app.include_router(jobs_v3.router, prefix="/api/v3")
app.include_router(files_v3.router, prefix="/api/v3")
app.include_router(credentials_v3.router, prefix="/api/v3")
app.include_router(status_v3.router, prefix="/api/v3")
...
# register after dedup: pre-dedup inclusion shadows /api/v2/auth/get_user (dedup compares prefix-less paths under _IncludedRouter)
app.include_router(admin_manager_v2.router, prefix="/api/v2/admin")

if should_mount_oidc():
    # issuer routes exist only when the deployment opted in (auth.mode=dual|oidc)
