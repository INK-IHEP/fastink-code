...
def _authorize(caller: Optional[str], token: Optional[str]) -> Optional[dict]:
    try:
        if not caller or not token or not headers.validate_token(caller, token):
            return _denied()
...
def get_user(
    ...
    if denied := _authorize(caller, token):
        return denied
    try:
