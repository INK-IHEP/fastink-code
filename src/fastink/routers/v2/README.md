...
<span style="color:red">这里如果有不需要认证的接口，请联系维护者更新需要跳过认证的列表。已知不需要认证的接口有：</span>
...
HTTP 返回状态值保持http服务原有定义。即https正常运行，状态码为200。业务是否正确，在返回值中三段式定义。
...
可参见： `src/fastink/routers/status.py`
