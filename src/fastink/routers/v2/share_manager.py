...
from fastink.common.utils import get_krb5cc, get_uname_from_uid
...
from fastink.sharefile.share_file import get_link
...
async def get_info(req: Request, path: str = Query(None)) -> dict:
    try:
        ...
        logger.debug("签名验证失败（密钥错误或令牌被篡改）")
    except jwt.InvalidTokenError as e:
        logger.debug("无效的令牌：%s", e)
