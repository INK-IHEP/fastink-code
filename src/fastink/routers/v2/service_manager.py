...
from fastink.service.code_datadir import ensure_code_datadir
...
# NOTE: /service/access_shared_rootfile is temporarily disabled.
# The whole route is commented out and its IP-whitelist entries were removed
# from deploy/templates/base/config.yml.tpl and
# fastink-dev/overlay/config.ihep.yml. Re-enable the route and whitelist
# entries together if shared ROOT file access is needed again.
...
@router.post("/service/code/ensure_datadir")
async def post_ensure_code_datadir(
    username: str = Depends(get_username),
) -> dict:
    """Ensure ~/.local/share/opencode lives on a non-AFS filesystem.

    Called by the frontend before submitting an opencode/openchamber job:
    AFS does not handle opencode's SQLite locking well, so on-AFS data is
    moved (or created fresh) under the scratch root and symlinked back.
    """
    try:
        result = await ensure_code_datadir(username)
        return {
            "status": InkStatus.SUCCESS,
            "msg": "Ensure code datadir successfully",
            "data": result,
        }
    except Exception as e:
        logger.error(
            f"Failed to ensure code datadir for user {username}: {str(e)}\n{traceback.format_exc()}"
        )
        return {
            "status": InkStatus.INTERNAL_ERROR,
            "msg": f"Failed to ensure code datadir: {str(e)}",
            "data": None,
        }
