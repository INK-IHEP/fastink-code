...
from fastink.common.utils import get_uname_from_uid
...
#### Delete a file
async def fileDelete(TargetPath:str, username:str) -> Tuple[bool,str]:
    ...
    try :
        ...
        if target_path[0:2] == '"\'' or target_path[0:2] == '\'"':
            ...
            msg = f"TargetPath {TargetPath} invalid"
            return (False, msg)

        logger.info(f"Processing fileDelete request {TargetPath}")
        ...
        if not is_exist:
            ...
            return (False, f"TargetPath {TargetPath} doesn't exist.")
        ...
        if not status or status is None:
            ...
            return (False, f"Failed to delete {TargetPath}.")
        logger.info(f"{TargetPath} is deleted.")
    except Exception as e:
        return (False, f"Failed to delete {TargetPath}. Error: {str(e)}")
    return (True,'')
...
async def delete_path(req: Request,
    ...
    try:
        body = await req.json()
        TargetPaths:List[str] = body['TargetPath']
    except Exception as e:
    ...
    bad_dnames: List[str] = []
    for name in TargetPaths:
        try:
            TargetPath = urllib.parse.unquote(name, encoding='utf-8')
            if not TargetPath:
                continue
            (status, msg) = await fileDelete(TargetPath=TargetPath,username=username)
            if not status:
                bad_dnames.append({"file":TargetPath, "error": msg})
        except Exception as e:
            bad_dnames.append((name, str(e)))
    if len(bad_dnames) == 0:
        return {"status": InkStatus.OK, "msg": "OK", "data": None}
    else:
        return {"status": InkStatus.FS_UNKNOWN_ERROR, "msg": "Failed to delete dir/files", "data": bad_dnames}
