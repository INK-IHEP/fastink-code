...
from fastink.computing.tools.common.utils import change_username_to_uid, get_user_jobs_dir
from fastink.computing.tools.common.failed_jobs import get_failed_job
from fastink.computing.apps import registry as computing_registry
from fastink.computing.job_prepare import (
    JobPreparationError,
    PrepareJobRequest,
    list_job_type_capabilities,
    prepare_job_request,
)
...
@router.get("/list_job_types")
async def list_job_types(
    request: Request,
    username: str = Depends(get_username),
    token: str = Depends(get_token),
):
    if not headers.request_authenticated(request) and (
        not username or not token or not headers.validate_token(username, token)
    ):
        return {
    ...
    try:
        return {
            ...
            "msg": "Request Success",
            "data": list_job_type_capabilities(),
        }
    except Exception as error:
        logger.exception("List job types failed, username=%s", username)
        return {
            "status": InkStatus.SERVER_INTERNAL_ERROR,
            "msg": f"List job types failed: {error}",
            "data": {},
        }


@router.post("/prepare_job")
async def prepare_job(
    ...
    if not headers.request_authenticated(http_request) and (
        not username or not token or not headers.validate_token(username, token)
    ):
        return {
            "status": InkStatus.TOKEN_INVALID,
    ...
    try:
        return {
            "status": InkStatus.SUCCESS,
            "msg": "Request Success",
            "data": prepare_job_request(
                request.job_type, request.cluster_id, request.overrides
            ),
        }
    except JobPreparationError as error:
        return {
        ...
        logger.exception("Prepare job failed, username=%s", username)
        return {
            "status": InkStatus.SERVER_INTERNAL_ERROR,
            "msg": f"Prepare job failed: {error}",
            "data": {},
        }
...
async def check_common_job(
    ...
    try:
        # Failed-submission records (epoch-ms pseudo ids from query_job's
        # FAILED entries) live in Redis, not in job output files: return
        # the stored failure reason as the job's error stream.
        r = redis_connect()
        failed_record = await get_failed_job(r, username, job_id)
        if failed_record:
            return {
                "status": InkStatus.SUCCESS,
                "msg": "Request Success",
                "data": {
                    "job_id": job_id,
                    "output": "",
                    "error": failed_record.get("failReason", ""),
                }
            }
...
@router.get("/get_user_jobs_dir")
async def get_user_jobs_dir_api(
    username: str = Depends(get_username),
    token: str = Depends(get_token),
):
    """Return per-cluster job-directory roots for the user.

    New jobs are created under ``<ink_dir>/.ink/Jobs/<cluster_id>/<jobType>-<ts>``
    so htcondor and slurm jobs live side-by-side but never intermixed
    (``ink_dir`` comes from the ``computing.ink_dir`` config template
    with ``{username}`` / ``{user_group}`` placeholders resolved).

    Response contains one field per active cluster in
    ``computing.cluster_list`` -- always at least ``htcondor_jobs_dir``,
    and ``slurm_jobs_dir`` when slurm is enabled.  Historical jobs
    created before this layout keep the paths recorded in the DB and
    are unaffected.
    """
    try:
        uid = change_username_to_uid(username)
        ...
        if isinstance(cluster_list, str):
            cluster_list = [c.strip() for c in cluster_list.split(",") if c.strip()]
        for cluster_id in cluster_list:
            data[f"{cluster_id}_jobs_dir"] = get_user_jobs_dir(username, uid, cluster_id)
        ...
        logger.error(f"Get user jobs dir failed, username={username}, details: {e}")
        return {
            "status": InkStatus.SERVER_INTERNAL_ERROR,
            "msg": f"Get user jobs dir failed: {e}",
            "data": {}
        }
