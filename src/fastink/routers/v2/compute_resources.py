...
async def get_user_jobs_dir_api(
    ...
    """Return the parent directory of the user's job directories.

    Job dirs are created as ``{jobs_dir}/{job_type}-{timestamp}``; this
    endpoint exposes the ``{jobs_dir}`` root (``<ink_dir>/.ink/Jobs``,
    where ink_dir comes from the ``computing.ink_dir`` config template
