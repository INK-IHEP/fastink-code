...
def register_hook(hook_name: str):
    ...
    Conflict semantics (uniform across hooks / apps / plugin routes):
    registration order equals load order, the LAST registration wins,
    and every override is logged as a warning so silent behaviour
    changes are visible in production logs.

    Args:
        hook_name: Name of the hook to register
    """
    def decorator(func: Callable) -> Callable:
        existing = _HOOKS_REGISTRY.get(hook_name)
        if existing is not None and existing is not func:
            logger.warning(
                "Hook override: %r previously registered by %s.%s, "
