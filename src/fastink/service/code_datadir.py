...
@hookable
async def post_datadir_setup(username: str, user_path: str, result: dict) -> dict:
    """Post-migration hook run after ~/.local/share/opencode is ensured
    off AFS. Sites may attach app-specific setup here (e.g. IHEP injects
    the hepai API key into auth.json). Default implementation is a no-op.
    """
    return result
...
async def ensure_code_datadir(username: str) -> dict:
    ...
    if state == RESOLVED_OK:
        ...
        return await post_datadir_setup(username, user_path, {
            "username": username,
            "group_dir": group_dir,
            "target": target,
            "migrated": False,
