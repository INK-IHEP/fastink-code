...
def _get_openclaw_templates() -> dict:
    """Provider templates offered in the OpenClaw UI.

    Site-specific templates come from the service.openclaw_provider_templates
    config key (a mapping with the same shape as DEFAULT_OPENCLAW_TEMPLATES)
    and are listed before the built-in defaults; on key conflicts the site
    template wins.
    """
    site_templates = get_config("service", "openclaw_provider_templates", fallback={}) or {}
    templates = dict(site_templates)
    for key, value in DEFAULT_OPENCLAW_TEMPLATES.items():
        if key not in templates:
            templates[key] = value
    return templates


def _get_default_allowed_origins() -> list:
    """Site UI origins always allowed for OpenClaw control (config-driven)."""
    origins = get_config("service", "openclaw_allowed_origins", fallback=[]) or []
...
def _is_local_model(base_url: str, model_id: str) -> bool:
    local_base_url = get_config("service", "openclaw_local_model_base_url", fallback="")
    if not local_base_url:
        return False
    local_prefix = get_config("service", "openclaw_local_model_prefix", fallback="")
    return (
        _normalize_base_url(base_url) == _normalize_base_url(local_base_url)
        and (not local_prefix or str(model_id or "").startswith(local_prefix))
    )
...
async def _get_user_model_api_key(username: str) -> str:
    try:
        api_key = await get_user_api_key(username=username)
...
def _render_openclaw_templates(
    ...
    templates = _get_openclaw_templates()
    selected_key = selected_key if selected_key in templates else None
    ordered_keys = []
    if selected_key:
        ordered_keys.append(selected_key)
    ordered_keys.extend(
        key for key in templates.keys() if key != selected_key
    )

    rendered = {}
    for key in ordered_keys:
        entry = deepcopy(templates[key])
...
def _template_key_for_model(base_url: str, model_id: str) -> str:
    ...
    for key, template in _get_openclaw_templates().items():
        if key == "custom":
            continue
        if (
...
def _validate_experiment_data_models(target_config: dict) -> None:
    ...
    if invalid_models:
        raise ValueError(
            "add_experiment_data=true requires all OpenClaw models to use site-local models only. "
...
def _ensure_default_allowed_origins(control_ui: dict) -> list[str]:
    ...
    allowed_origins = []
    for origin in existing_allowed_origins + _get_default_allowed_origins():
