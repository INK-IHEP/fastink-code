...
def replace_https(url):
    """Rewrite a legacy public URL prefix to the current nginx node.

    service.legacy_url_prefix holds the historical prefix that may be
    embedded in stored URLs; empty means no rewriting is needed.
    """
    legacy_prefix = get_config("service", "legacy_url_prefix", fallback="")
    nginx = get_config("computing", "nginx_node", fallback="")
    if not legacy_prefix or not nginx:
        return url
    return url.replace(legacy_prefix, f"{nginx}")


def remote_ssh_connect():
    SERVICE_NODE = get_config(
        "service", "service_node", fallback="fastink-rootbrowse"
    )
    SERVICE_PORT = get_config("service", "service_port", fallback=2000)
    client = paramiko.SSHClient()
    client.set_missing_host_key_policy(paramiko.AutoAddPolicy())

    key_path = get_config("service", "ssh_private_key", fallback="/root/.ssh/id_rsa")
...
def push_root_script(ssh: paramiko.SSHClient):
    ...
    sftp = ssh.open_sftp()
    try:
        sftp.put("src/fastink/service/scripts/start-rootbrowse.sh", RBSCRIPT)
        sftp.put("src/fastink/service/scripts/check-rootbrowse.sh", RBCSCRIPT)
