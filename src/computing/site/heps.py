...
async def build_job_env(uid, jobtype, rawjobPath, jobfilename):
    ...
    with log_step("resolve_user_info", logger=build_env_logger):
        username = change_uid_to_username(uid)
        _, user_group = get_user_exp_group(uid)
        
    with log_step("resolve_paths", logger=build_env_logger):
    ...
    if krb5_enabled:
        with log_step("krb5_get_token", logger=build_env_logger):
        ...
        with log_step("write_krb5_token_file", logger=build_env_logger):
            token_filename = f"/tmp/krb5cc_{uid}_{time_stamp}"
            if not os.path.exists(token_filename):
                with open(token_filename, 'wb') as file:
                    file.write(krb5_decoded_bytes)
        
        with log_step("xrootd_prepare_job_dir", logger=build_env_logger):
        ...
        with log_step("xrootd_upload_krb5_token", logger=build_env_logger):
            job_dir = f"{user_home_dir}/.ink/Jobs/{jobtype}-{time_stamp}"    
    ...
    with log_step("xrootd_chmod_job_script", logger=build_env_logger):    
...
async def submit_hpc_job(sbatch_command, job_type, job_path, uid):
    with log_step("resolve_user_info", logger=sbatch_logger):
    ...
    with log_step("execute_sbatch", logger=sbatch_logger):
        stdout = await sub_command(command, 30, "submit job failed.", "submit job timeout.")
    
    with log_step("parse_job_id", logger=sbatch_logger):
