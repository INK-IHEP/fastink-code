...
async def build_job_env(uid, jobtype, rawjobPath, jobfilename):
    ...
    with log_step("resolve_user_info", build_env_logger):
        username = change_uid_to_username(uid)
        _, user_group = get_user_exp_group(uid)
        
    with log_step("resolve_paths", build_env_logger):
    ...
    if krb5_enabled:
        with log_step("krb5_get_token", build_env_logger):
    ...
    with log_step("xrootd_chmod_job_script", build_env_logger):    
...
async def submit_hpc_job(sbatch_command, job_type, job_path, uid):
    with log_step("resolve_user_info", sbatch_logger):
    ...
    with log_step("build_sbatch_command", sbatch_logger):
        command = (
    ...
    with log_step("execute_sbatch", sbatch_logger):
        stdout = await sub_command(command, 30, "submit job failed.", "submit job timeout.")
    
    with log_step("parse_job_id", sbatch_logger):
