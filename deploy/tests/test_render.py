...
class TestBuildMapping:
    ...
    def test_cluster_list(self, sample_answers, sample_paths):
        ...
        assert mapping["noenv_jobtype"] == ["jupyter", "vnc"]
        assert len(mapping["start_keywords"]) == 6
        assert "vscode" in mapping["jobtype_defaults"]
        assert mapping["jobtype_defaults"]["vscode"]["htc"]["RequestMemory"] == 6000
...
class TestRenderBundle:
    ...
    def test_jobtype_defaults_in_config(self, sample_answers, sample_paths, tmp_path):
        """Verify jobtype_defaults are rendered into config.yml via template for loop."""
        deploy_dir = tmp_path / "deploy"
        deploy_dir.mkdir()
        bundle = render_bundle(
            "quickstart", sample_answers, sample_paths, deploy_dir,
