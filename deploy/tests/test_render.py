...
class TestRenderConfig:
    ...
    def test_test_account_section_defaults(self, sample_answers, sample_paths):
        # The test account section must exist in the base template so site
        # overlays only override (never introduce) it, and open-source
        # deployments can run the test suite without extra config.
        mapping = build_mapping("quickstart", sample_answers, sample_paths, Path(tempfile.mkdtemp()))
        result = render_config("quickstart", mapping)
        data = yaml.safe_load(result)
        assert data["test"] == {"username": "", "password": ""}
