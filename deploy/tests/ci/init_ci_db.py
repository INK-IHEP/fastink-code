...
counterpart to tools/init_database.py, which seeds only the root user
by default and reads site-specific users from an optional seed file
(see tools/README.md). Here we only create the schema plus the rows
the portable test layer depends on.
