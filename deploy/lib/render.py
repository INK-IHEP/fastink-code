...
from lib.defaults import default_image_answers
...
def _build_network_mapping(
    ...
    return {
        ...
        "node_domain_suffix": get_str(answers, "node_domain_suffix", ""),
...
def _build_storage_mapping(
    ...
    return {
        ...
        "server_log_dir": str(paths.get("server_log_dir", paths["data_root"] / "logs").resolve()),
    }


def _build_auth_mapping(enable_krb5: bool) -> dict[str, str]:
...
def _build_computing_mapping(
    ...
    The former ``noenv_jobtype`` / ``start_keywords`` / ``iptables_jobtype``
    / ``jobtype_defaults`` lists have been removed: their content is now
    owned by the JobApp classes under ``fastink.computing.apps`` and read
    at runtime through ``fastink.computing.apps.registry``.
    """
...
def build_mapping(
    ...
    *,
    ...
    # ---- computing defaults ----
    # start_keywords / noenv_jobtype / iptables_jobtype / jobtype_defaults
    # are no longer produced here: their content is owned by the JobApp
    ...
    mapping.update(_build_auth_mapping(enable_krb5))
    mapping.update(_build_computing_mapping(
        deploy_dir, schedd_host, cm_host, enable_local_htcondor,
        htcondor_internal_domain, cluster_list,
    ))
    ...
    image_defaults = default_image_answers(get_str(answers, "image_source", "pull"))
    ...
    mapping.update({
        ...
        "htcondor_image": get_str(answers, "htcondor_image", image_defaults["htcondor_image"]),
        "xrootd_image": get_str(answers, "xrootd_image", image_defaults["xrootd_image"]),
...
def render_nginx_conf(mapping: dict[str, str]) -> dict[str, str]:
    """Render the nginx skeleton, base snippets, and per-app locations.

    Sources:
      * ``deploy/templates/base/nginx/**``          -- server / snippets /
        the site-wide fallback location (proxy-upstream.conf);
      * ``src/fastink/computing/apps/<name>/nginx.location.conf`` --
        the per-app reverse-proxy snippet, colocated with the app's
        run.sh and connect.py.

    App-owned files land under ``nginx/locations/<name>.conf`` in the
    rendered bundle so nginx's ``include locations/*.conf`` directive
    ...
    # Per-app nginx snippets shipped alongside the app modules.
    apps_dir = Path(__file__).resolve().parents[2] / "src" / "fastink" / "computing" / "apps"
    if apps_dir.is_dir():
        for snippet in sorted(apps_dir.glob("*/nginx.location.conf")):
            name = snippet.parent.name
            bundle[f"nginx/locations/{name}.conf"] = render_template_text(snippet, mapping)
