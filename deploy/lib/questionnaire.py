...
def build_paths_from_answers(answers: dict[str, object], deploy_dir: Path) -> dict[str, Path]:
    ...
    _, paths = build_runtime_paths(
        ...
        server_log_dir=Path(answers["server_log_dir"]).resolve() if answers.get("server_log_dir") else None,
    )
...
def collect_answers_custom(previous_answers: Optional[dict[str, object]], deploy_dir: Path) -> dict[str, object]:
    ...
    image_label = "tag" if image_source == "build" else "reference"
    server_image = cli_ui.text_prompt(f"Server image {image_label}", str(image_defaults["server_image"]), str(r("server_image")) if r("server_image") else None)
    cron_image = cli_ui.text_prompt(f"Cron image {image_label}", str(image_defaults["cron_image"]), str(r("cron_image")) if r("cron_image") else None)
    rootbrowse_image = cli_ui.text_prompt(f"Rootbrowse image {image_label}", str(image_defaults["rootbrowse_image"]), str(r("rootbrowse_image")) if r("rootbrowse_image") else None)
    ...
    if enable_local_htcondor:
        htcondor_image = cli_ui.text_prompt(f"HTCondor image {image_label}", htcondor_image_default, str(r("htcondor_image")) if r("htcondor_image") else None)
    ...
    # NOTE: per-jobtype CPU/memory defaults now live on the JobApp classes
    # under fastink.computing.apps.<name>.request_defaults (site overrides
    # go in fastink-dev/overlay/config.ihep.yml jobtype:<name>.htc).  The
    # old global "Default HTCondor job CPUs / memory" prompts were removed
    # because their answers were silently ignored by the render layer.
