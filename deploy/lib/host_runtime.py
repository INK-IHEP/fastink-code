...
# Default CVMFS paths warmed during the pre-deploy check. The list is a
# convention for HEP sites (CERN sft + IHEP software repos are publicly
# accessible); override it per deployment with the ``cvmfs_required_paths``
# answers key (a list of absolute paths; empty list = use these defaults).
DEFAULT_REQUIRED_CVMFS_PATHS = (
...
def resolve_required_cvmfs_paths(configured: Iterable[str] | None = None) -> tuple[Path, ...]:
    """Return the CVMFS paths to warm: configured values or the defaults."""
    paths = tuple(Path(p) for p in (configured or []) if str(p).strip())
    return paths or DEFAULT_REQUIRED_CVMFS_PATHS


def warm_cvmfs_paths(paths: Iterable[Path] | None = None) -> None:
    for path in (paths if paths is not None else DEFAULT_REQUIRED_CVMFS_PATHS):
        warm_cvmfs_path(path)



def check_host_prerequisites(*, require_cvmfs: bool = True, cvmfs_paths: Iterable[str] | None = None) -> None:
    require_command('docker')
    require_command('mountpoint')
    require_docker_compose()
    if require_cvmfs:
        require_cvmfs_mount()
        warm_cvmfs_paths(resolve_required_cvmfs_paths(cvmfs_paths))
