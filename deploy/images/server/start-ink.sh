...
install_plugin_editable_dirs() {
    ...
    for directory in "${dir_list[@]}"; do
        ...
        # --no-build-isolation: pip 26.2's isolated build-deps subprocess
        # crashes at startup once site-packages holds a PEP 660 editable
        # .pth (stdlib falls off sys.path). The image pins its own
        # setuptools/wheel toolchain, so isolation buys nothing here.
        pip3.12 install --no-build-isolation -e "$directory"
    done
}
...
if [[ "${INSTALL_EDITABLE:-false}" == "true" ]]; then
    pip3.12 install --no-build-isolation -e /ink
fi
...
if [[ "${INIT_DATABASE_ON_START:-true}" == "true" ]]; then
    ...
    # Idempotent schema migrations run here so every container start on a
    # new image brings the DB to the schema that image's code expects.
    python3.12 /ink/tools/migrate_drop_email.py
fi

if [[ "${INK_PRODUCTION:-false}" == "true" ]]; then
    exec python3.12 -m uvicorn fastink.main:app --workers "$WORKERS" --host "$HOST" --port "$PORT" --no-access-log
fi

exec python3.12 -m uvicorn fastink.main:app --reload --host "$HOST" --port "$PORT" --no-access-log
