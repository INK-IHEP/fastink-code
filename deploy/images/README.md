...
- keep image definitions in `fastink-code`
...
- site deployments should consume these image definitions through `fastink-code/deploy` and `fastink-dev` overlays
...
`fastink-code/.gitlab-ci.yml` is the publishing pipeline for these images.
...
- `fastink-code` triggers `fastink-dev` with full image references
