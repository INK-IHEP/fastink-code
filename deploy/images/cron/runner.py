...
LOG_DIR = Path(os.environ.get("FASTINK_CRON_LOG_DIR", "/ink/log"))
...
def _log(msg: str) -> None:
    ...
    if _bootstrap_logger is None:
        ...
        _bootstrap_logger = logging.getLogger("fastink-cron.bootstrap")
    ...
    if dirs.strip():
        for d in dirs.split(","):
            ...
            if d:
                ...
                # --no-build-isolation: same pip 26.2 regression as the
                # server entrypoint — isolated build subprocesses crash at
                # startup once site-packages holds a PEP 660 editable .pth.
                subprocess.run(
                    [sys.executable, "-m", "pip", "install", "--no-build-isolation", "-e", d],
                    check=True, capture_output=True,
                )
...
def install_fastink_editable() -> None:
    if os.environ.get("INSTALL_EDITABLE", "false").lower() == "true":
        ...
        if INK_CODE_DIR.is_dir():
            subprocess.run(
                [sys.executable, "-m", "pip", "install", "--no-build-isolation", "-e", str(INK_CODE_DIR)],
                check=True, capture_output=True,
            )
        else:
...
async def main() -> None:
    ...
    # Activate unified plugins and hooks (same bootstrap as the API
    # server) so cron jobs see the same overridden behavior, e.g. site
    # accounting ClassAds in generated HTCondor submit files.
    try:
        # install_plugins() ran pip inside this same process's lifetime;
        # editable installs add path entries that Python's import system
        # has already cached past. Invalidate caches and re-scan
        # site-packages so the freshly installed plugin is importable.
        import importlib
        import site
        site.main()
        ...
        _log(f"ERROR: plugin/hook bootstrap failed: {e}")
        sys.exit(1)
