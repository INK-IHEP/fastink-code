...
def check_cvmfs(answers: DeployAnswers) -> None:
    # CVMFS check: warn but don't block deployment. Paths can be
    # customized per deployment via the cvmfs_required_paths answer.
    try:
        check_host_prerequisites(
            require_cvmfs=True,
            cvmfs_paths=answers.get("cvmfs_required_paths") or None,
        )
    except RuntimeError as exc:
...
def run_init_container(answers: DeployAnswers, paths: dict[str, Path]) -> None:
    ...
    cmd = [
        ...
        "-e",
        f"FASTINK_ENABLE_HOST_SLURM_CLIENT={'true' if answers.get('enable_host_slurm_client') else 'false'}",
        "-e",
