...
This document defines how `fastink-code` publishes official images and how
...
- `fastink-code`
  ...
  - provide shared base/profile templates and render logic
  - trigger downstream deployment in `fastink-dev`
- `fastink-dev`
  - keep only the IHEP overlay
  - render `base + custom + ihep`
...
`fastink-code/.gitlab-ci.yml` triggers `fastink-dev` and passes image names with:
...
1. rendered custom profile from `fastink-code/deploy`
2. IHEP compose overlay
3. filesystem mount overlay

## Required CI variables

For `fastink-code` publishing:
...
1. Build all three images locally from `fastink-code`.
