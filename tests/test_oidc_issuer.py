...
def test_device_approval_requires_authenticated_login(client, monkeypatch):
    created = client.post("/device_authorization", data={"client_id": AUDIENCE}).json()
    ...
    assert response.status_code == 200
    assert "Device approved" in response.text
    token = client.post("/token", data={"grant_type": "urn:ietf:params:oauth:grant-type:device_code", "device_code": created["device_code"], "client_id": AUDIENCE}).json()
    claims = jwt.decode(token["access_token"], options={"verify_signature": False})
    assert claims["preferred_username"] == "alice"
