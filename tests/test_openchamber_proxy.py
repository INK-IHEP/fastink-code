...
def test_latest_login_info_does_not_try_older_directory(monkeypatch):
    ...
    async def fake_path_exist(**kwargs):
        # The helper now (a) checks each scope dir exists before listing
        # and (b) checks the login-info file inside the newest candidate.
        if kwargs["name"] == "/jobs":
            return True, "dir"
        assert kwargs["name"] == (
...
def test_latest_login_info_reads_newest_directory(monkeypatch):
    ...
    async def fake_path_exist(**kwargs):
        # Scope-dir existence probe first, then the login-info file check.
        if kwargs["name"] == "/jobs":
            return True, "dir"
        assert kwargs["name"] == (
            "/jobs/openchamber-20260730-120000/app_login.info"
