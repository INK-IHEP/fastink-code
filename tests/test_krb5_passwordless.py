...
class TestCcacheEncryption:
    def test_encrypt_decrypt_roundtrip(self, monkeypatch):
        from cryptography.fernet import Fernet
        key = Fernet.generate_key().decode()
        monkeypatch.setattr(
            krb5, "get_config",
            lambda s, o, **kw: {"ccache_encrypt_key": key} if (s, o) == ("auth", "delegation") else kw.get("fallback"),
        )
        token = "dGVzdCBjY2FjaGU="
        encrypted = krb5._encrypt_ccache(token)
        assert encrypted != token
        assert krb5._decrypt_ccache(encrypted) == token

    def test_encrypt_without_key_raises(self, monkeypatch):
        monkeypatch.setattr(
            krb5, "get_config",
            lambda s, o, **kw: {"ccache_encrypt_key": ""} if (s, o) == ("auth", "delegation") else kw.get("fallback"),
        )
        with pytest.raises(RuntimeError, match="ccache_encrypt_key"):
            krb5._encrypt_ccache("token")
    ...
    def test_decrypt_without_key_raises(self, monkeypatch):
        ...
        with pytest.raises(RuntimeError, match="ccache_encrypt_key"):
            krb5._decrypt_ccache("token")

    def test_persist_encrypts_before_saving(self, stub_user, monkeypatch):
        from cryptography.fernet import Fernet
        key = Fernet.generate_key().decode()
        monkeypatch.setattr(
            krb5, "get_config",
        ...
        saved = []
        monkeypatch.setattr(krb5, "save_token", lambda **kw: saved.append(kw))
        token = _ccache_token(
            endtime=int((datetime.now() + timedelta(hours=10)).timestamp()),
            renew_until=int((datetime.now() + timedelta(days=7)).timestamp()),
        )
        krb5._persist_ccache_token("alice", token)
        assert len(saved) == 1
        assert saved[0]["token"] != token
        assert krb5._decrypt_ccache(saved[0]["token"]) == token
