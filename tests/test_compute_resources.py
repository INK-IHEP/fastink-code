...
def test_delete_job_requires_job_id_or_submit_uuid(monkeypatch):
    from fastink.routers.v2 import compute_resources
    ...
    response = client.post(
        ...
        headers={"Ink-Username": "alice", "Ink-Token": "valid-token"},
        json={"cluster_id": "slurm"},
    )

    assert response.status_code == 200
    data = response.json()
    # Empty job_id + submit_uuid is a parameter error, not an internal error.
    assert data["status"] == InkStatus.PARAM_ERROR
    assert data["data"] == {}
